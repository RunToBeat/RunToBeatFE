...
function StoryHome() {
  ...
  const [currentMainImage, setCurrentMainImage] = useState(
    require('../../image/BigEpisode1.png'), // 초기 메인 이미지
  );

  const handleMainImagePress = () => {
    if (currentEpisodeId === 1) {
      navigation.navigate('Situation'); // 1화 클릭 시 'Situation'으로 이동
    } else if (currentEpisodeId === 2) {
      Alert.alert('Coming soon', '스토리 준비 중입니다.');
    } else {
      Alert.alert('', '이전 에피소드를 완료해야 달릴 수 있습니다.');
    }
  };

  const handleEpisodeImagePress = episode => {
    setCurrentEpisodeId(episode.id);
    setCurrentMainImage(episode.mainImage); // 클릭한 에피소드의 메인 이미지로 변경
  };
  ...
  const episodes = [
    ...
    },
    {
      ...
      showOverlay: false, // 자물쇠 이미지가 보이도록 설정
    },
    {
      id: 3,
      title: '3화. 임시정부의 활동',
      ...
      mainImage: require('../../image/episode2.png'), // 에피소드 별 mainImage 추가
      showOverlay: true, // 자물쇠 이미지가 보이도록 설정
    },
    {
      id: 4,
      title: '4화. 중국으로',
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <TouchableOpacity onPress={handleMainImagePress}>
        <Image
          style={styles.mainImage}
          source={currentMainImage} // 현재 에피소드의 mainImage 표시
        />
      </TouchableOpacity>
      ...
      <View style={styles.episodeListContainer}>
        {episodes.map(episode => (
          <TouchableOpacity
            key={episode.id}
            onPress={() => handleEpisodeImagePress(episode)}>
